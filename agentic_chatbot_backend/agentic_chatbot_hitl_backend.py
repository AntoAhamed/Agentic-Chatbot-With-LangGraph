...
# ticket booking tool (dummy tool for HITL concept)
@tool
def book_ticket(symbol: str, quantity: int) -> dict:
    """
    Simulate booking a given quantity of a ticket symbol.

    HUMAN-IN-THE-LOOP:
    Before confirming the booking, this tool will interrupt
    and wait for a human decision ("yes" / anything else).
    """
    # This pauses the graph and returns control to the caller
    decision = interrupt(f"Approve booking {quantity} tickets of {symbol}? (yes/no)")

    if isinstance(decision, str) and decision.lower() == "yes":
        return {
            "status": "success",
            "message": f"Booking order placed for {quantity} tickets of {symbol}.",
            "symbol": symbol,
            "quantity": quantity,
        }
        ...
        return {
            "status": "cancelled",
            "message": f"Booking of {quantity} tickets of {symbol} was declined by human.",
            "symbol": symbol,
            "quantity": quantity,
        }
...
# tools list
tools = [search_tool, calculator, get_weather_data, rag_tool, book_ticket]
